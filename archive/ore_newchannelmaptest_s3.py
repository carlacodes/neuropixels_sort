...
def spikesorting_pipeline(rec_name, params):
    ...
    sorting_output = ss.run_sorters(params['sorter_list'], [recording], working_folder=working_directory,
                                    ...
                                    engine='loop', sorter_params={'pykilosort': {'n_jobs': 13, 'chunk_size': 30000}},
                                    verbose=True)
...
def spikesorting_postprocessing(params, step_one_complete=False):
    ...
    if step_one_complete == False:
        ...
        for (rec_name, sorter_name), sorting in sorting_output.items():
            ...
            we = sc.extract_waveforms(sorting._recording, sorting, outDir / 'waveforms_folder_sparse4',
                    # load_if_exists=True,
                    overwrite=False,
                    ms_before=2, 
                    ms_after=3., 
            ...
            metrics = compute_quality_metrics(we, metric_names=['d_prime'], n_jobs=13)
            # metrics = compute_quality_metrics(we)
            logger.info('Export report')
            print('exporting report')
            sexp.export_report(we, outDir / 'report_dprime', format='png', force_computation=False, **jobs_kwargs)
            #n_jobs = 8, chunk_size=3000
            # logger.info(f'Exporting to phy')
            # sexp.export_to_phy(we, outDir / 'phy', remove_if_exists=True,
            #                 verbose=True, copy_binary=False,
...
def main():
    ...
    params_file = Path('/home/zceccgr/Scratch/zceccgr/neuropixelsdecodingproject/params/oreparams_s3.json')  # 'params/params.json
    ...
    #     logger.info('using concurrent futures')
    #     futures = [executor.submit(ss.run_sorters, params['sorter_list'], [recording], working_folder=params['working_directory'], mode_if_folder_exists='keep', engine='loop', sorter_params={'pykilosort': {'n_jobs': 13, 'chunk_size': 30000}}, verbose=True)]
